...
$.fn.extend({
    ...
    remove_tree: function (o) {
        let isAddingSubblock = $("a.active")[0].innerText == $(".wfid_add_block").val();
        let parentA;
        if (isAddingSubblock) {
            parentA =  $("a.active");
            parentA.siblings("ul").remove();
        } else {
            var clickedParentUl = $("a.active").closest("ul")
            parentA = clickedParentUl.parent().closest("ul").find("a:first");
            clickedParentUl.siblings("ul").remove();
            clickedParentUl.remove();
        }
        $(".tree .wfid_updating_subtree").toggleClass("wfid_updating_subtree");
        parentA.removeClass("treed");
        parentA.addClass("wfid_updating_subtree_lvl1")
        parentA.click();
        setTimeout(()=> 
            {parentA.siblings("ul").treed();
            $(".wfid_updating_subtree_lvl1").toggleClass("wfid_updating_subtree_lvl1");}, 
            500);
